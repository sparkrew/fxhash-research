...
class InstanceFractalManager{
	...
	constructor(){
		...
		this.gcol1 = color(genR(100, 255), genR(100, 255), genR(100, 255));
		this.gcol2 = color(genR(100, 255), genR(100, 255), genR(100, 255));
		this.gcol3 = color(genR(100, 255), genR(100, 255), genR(100, 255));
		this.gcol4 = color(genR(100, 255), genR(100, 255), genR(100, 255));
	...
	update() {
		if (mouseIsPressed) {
			this.addCoso(mouseX, mouseY,1);
		}
	}
	addCoso(_x, _y, _s) {
		console.log("agrega coso");
		console.log(_s);
	  ...
	  */
		...
		gcol1 = color(genR(100,255), genR(100,255), genR(100,255) );
		gcol2 = color(genR(100,255), genR(100,255), genR(100,255) );
		gcol3 = color(genR(100,255), genR(100,255), genR(100,255) );
		gcol4 = color(genR(100,255), genR(100,255), genR(100,255) );
	  ...
	  var cnt = 1;
		...
		for (var i = 0; i < cnt; i++) {
		  ...
		  coso = new IntanceFractalParticle(_x,_y, 0);
...
class IntanceFractalParticle {
  ...
  constructor(x,y, _pas) {
	...
	var stspeed = random(10000);
	 ...
	 let st = 1;
	this.speed = createVector(genR(-st,st),genR(-st, st));
   ...
   // this.size1 = random(uniforms_fxhash.size1);
	...
	this.asize = map(this.pasadas, 0, maxpasadas, 15, 5);
   }
   display(_ps) {
    /*this.p1.noFill();
    this.p1.noStroke();
   ...
   // strokeWeight(map(this.pasadas,0,maxpasadas,0,2));*/
	...
	_ps.noStroke();
	_ps.fill(0,200);
    _ps.ellipse(this.ap.x, this.ap.y, this.asize * 1.1, this.asize * 1.1);
 ...
 //   bacol = color(255-red(bacol),255-green(bacol),255-blue(bacol));
    ...
    //asize = 3;
	 ...
	 // var rdm = map(this.pasadas, 0, maxpasadas, uniforms_fxhash.rdm * 0.5, uniforms_fxhash.rdm*3.8);
	  ...
	  this.life -= map(this.pasadas, 0, maxpasadas, 4,15);

	  var rdm = map(this.pasadas, 0, maxpasadas, 1. * 0.1, 1.*0.2);
	  rdm = 0.0;
