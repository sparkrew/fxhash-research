...
let tm = 10000
let randrl = [100, 200, 300];
let drl;
let chngjam = fxdouble(0,4)
let coinflip
//start
function setup() {
  ...
  //let windthran = random(500,900);
  //let heightran = random(500,900);

    createCanvas(random(400,450), random(660,700));

  //createCanvas(random(500,900), random(400,700));
  background(170);
  colorMode(HSB,255)
  t = 0 ; cont = 1
...
function noiseDW(x,y) {
  let ah1 = x/0.5;
  let ah2 = x*0.5;
  let funcifab = [ah1, ah2, ah2]
  let respul = funcifab[Math.floor($fx.rand()*funcifab.length)]
  return(noise(respul,y,2*noise(respul,y*2+2)))
}
  ...
  if (enabled){
    ...
    if (t % 400 < 100)
      for(c=0;c<3000;c++)
      {
        ...
        let distance = random(width / chngjam);
        let angle = random(TWO_PI);
        x0 = width / 2 + distance * cos(angle);
        y0 = height /2 + distance * sin(angle) ;
        if (coinflip == 0){
          const offsetWidth = width * 0.05;
          const offsetHeight = height * 0.05;
          x0 = constrain(x0, width / 4 + offsetWidth, 3 * width / 4 - offsetWidth);
          y0 = constrain(y0, height / 4 + offsetHeight, 3 * height / 4 - offsetHeight);
        }
        else{
          const offsetWidth = width * 0.04;
          const offsetHeight = height * 0.04;
          x0 = constrain(x0, width / 4 + offsetWidth, 3 * width / 4 - offsetWidth);
          y0 = constrain(y0, height / 4 + offsetHeight, 3 * height / 4 - offsetHeight);
        }
        ...
        for(cont = 1; cont < 900; cont++)
        {
          ...
          if (abs(h0 - h1) < .10 * abs(h0 - noiseDW((x0 - l * cos(d)) / 400, (y0 + l * sin(d)) / 400)))
          {

            stroke(color(d / PI * 256, h0 > colr ? 175 - (h0 - .5) * 500 : 20, h0 > .5 ? 50 * log((h0 - .5) * 200) : 200 + h0 * 200));
            //fill(color( 150 + h0 * 105, 200 - (h0 * 150), 150 + (1 - h0) * 105));
            // randrl[Math.floor($fx.rand()*randrl.length)]
            strokeWeight(0.1);
            line(x0, y0, x1, y1);
          }
          else cont = 900;
