...
{   
  ...
  //  p2.z+=time*5.;
   ...
   // p2.xy *= rotate2d(time * 2.+sin(p.z*.8+time));
/*    
...
float de(vec3 p) {
    ...
    //p.yz *= rot(pi/4.+sin(time*.3)*2.); // rotaciones
  //  p.xz *= rot(time * .02);
   // p.xy *= rot(time * .02+cos(time*.025)*5.);
   // p.xz *= rot(time*.001);
    ...
    float ms = floor(mapr(frc,4.0,15.))+1.;
  ...
  //  p.xy *= rot(time*.1+id2);
   ...
   /* if(idx2 > 0.5){
        oct = sdSphere(p,0.90+sin(id2*100.+time)*0.3);
    }else{
        oct = sdSphere(p,0.90+sin(id2*100.+time)*0.3);
    }*/
...
/*
    ...
    p2.x+=px;
    p2.y+=py;*/
    vec3 p3 = p2+vec3(px,py,pz);
    float d2 = max(d,1.-sdSphere(p3,20.));
...
void main(void)
{
    ...
    vec3 from = vec3(0.+px, 0.+py, -25.+pz);
	...
	vec3 p1 = vec3(sin(time) * 8.+px, sin(time * 0.)+py, cos(time) * 4.+pz); // definimos la posición de las luces
	vec3 p2 = vec3(mapr(mouse.x,-12.,12.)+px,mapr(mouse.y,8.,-8.)+py, cos(time) * 4.+pz);
 ...
 //  lightpos2 = -lightpos1.yxz; // copia las coordenadas de la luz 1 rotando los ejes
   ...
   /* if(mousePressed == 1){
	   	 lightpos1 = vec3(mapr(mouse.x,-12.,12.),mapr(mouse.y,8.,-8.), cos(time) * 4.); // definimos la posición de las luces
    }*/
