...
void main() {
	...
	mat4 rotatedInstanceMatrix = instanceMatrix + snoise(position.xyz * 3.0 * ${fxrand()} + frame*0.01)*0.03;
...
void main() {
    ...
    float ridx = idx*0.001*${fxrand()};
    float rid = id*0.001*${fxrand()};
	gl_FragColor = vec4(
        snoise(vec3(rid+uUv.x*3.0*${fxrand()}, rid+uUv.y*1.0*${fxrand()}, frame*0.6*${fxrand()}))*${fxrand()}*ridx+0.5,
        snoise(vec3(rid+uUv.y*5.0*${fxrand()}, rid+uUv.y*1.3*${fxrand()}, frame*0.6*${fxrand()}))*${fxrand()}*rid+0.5,
        snoise(vec3(rid+uUv.y*7.0*${fxrand()}, ridx+uUv.x*3.0*${fxrand()}, frame*0.6*${fxrand()}))*${fxrand()}*(ridx+rid)+0.5,
        1.0
        ); //color;
}
`
