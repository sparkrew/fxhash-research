...
void main(void){   
    ...
    for (int i =0; i<cnt; i++){
        ...
        vec2 uv2 = uv; 
        float fc = float(i+1)*1.;
      //  dib+= fbm(uv2*fc,time);

        float rxr1 = rxr(uv2);
        float rxr2 = fbm(uv2,time*.2+20.)*1.0;
        float e3 = fbm(uv2*fc*0.5,time*.2+100.);
        float e2 = fbm(uv2*fc*2.,time*.2+100.);
        float e = fbm(uv2*fc+e2,time*.2+100.);
       ...
       /*if(bloodsky == 0.0){
        ...
        vec3 c23 = mix(c2,c3,smoothstep(0.2,0.6,uv.y));
