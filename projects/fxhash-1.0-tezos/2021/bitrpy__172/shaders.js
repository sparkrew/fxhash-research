...
{
    vec2 uv = vUv;
    vec4 nr = vec4(rand(uv.x * time)); //vec4(sin(uv.x*100.0)*10.0*sin(uv.x*300.0*uv.y*1000.0));
    float c = (nr.r + nr.b + nr.g) / 3.0;

    if (rtime < 1.0) {
        gl_FragColor = vec4(1.0);
        return;
    };
    vec4 b = texture2D(canvas, uv);
    float rrand = rand(uv.x*uv.y*time*0.001*${fxrand()});
    float noisee = snoise(vec3(uv.x*10.3*${fxrand()}, uv.y*30.0*${fxrand()}, time*0.0001));// - 0.5;
    //if (noisee > 0.3) discard;

    if (uv.x > 0.45 && uv.y > 0.45 && uv.x < 0.55 && uv.y < 0.55) { 
        gl_FragColor = vec4(snoise(vec3(uv.x*400. * ${fxrand()},uv.y*700.0 * ${fxrand()},time*0.003 * ${fxrand()}))) ;
    } else {
        if (rrand > 0.001) {
           vec4 bscl = texture2D(canvas, 
                (uv + 
                    vec2(
                        (noisee)*0.002*${fxrand()}+0.001, 
                        (noisee)*0.002*${fxrand()}+0.001)
                    ) * 0.99 + (0.005));
            gl_FragColor = bscl;
        } else {
    ...
    if(gl_FragColor.r + gl_FragColor.g + gl_FragColor.b > 1.5) {
        gl_FragColor = vec4(1.0);
    } else {
    ...
    /*
    float clicked = sign(iMouse.z);
    if (clicked > 0.0) {
        if(abs(iMouse.x-fragCoord.x) < 2. && abs(iMouse.y-fragCoord.y) < 2.) {
