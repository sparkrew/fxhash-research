// preview hashes - 
//ooQ44ykQ5U3M3MHuhitorrZRJ8KGJSC247ncHaJyPzVrfvHL9LH
//oo9ckXuieCANoRiQNZ4NqXr967eT5U9pPK7zgGkjG6bqPdWGe2K
//oo8esChTZ2jqQ2BCVSPiBDan5FUGTVP2frULU43nUkg5U4uRJsE
//ooR2KLeV6XEc5ptXnJFkAP3jg56nJ6S61SPawXxi4wMS5SNVfwY
//oovvahCshKjmTQMnErqWnLhTkSnYYoLLep4dwePx5qXT1VecJA4
//ooHew6MXJBZABMzDHyc2q4Uj6k5wjas3HQhxCvsy5239s26tkeL
//oo7KHJankmqv4TXgUV3EsNwXmKiBNKnL8WQgnXSQehBNabqY8Sy
//oo8FstpNFNcfLRaibzyTehvfG5TzvJNC29new3ZXmvfCVbSLtgy
//



let title = "A Simple Test"
let maxDrawSegment
let seedValue //= 4040611622 //377790836
let devKeyCodesEnabled = false
let generatorMode = 0
...
function seedTheSeed(_seedValue, _perfect, _zoom) {
  ...
  // const fw_qubeType = [{v:0,w:0},{v:1,w:0},{v:2,w:0},{v:3,w:0},{v:4,w:0},{v:5,w:0},{v:6,w:1}]; //debug
  ...
  const fw_drawStyle = [{v:0,w:10},{v:1,w:25},{v:2,w:30},{v:3,w:35},{v:4,w:0}]; 
  // const fw_drawStyle = [{
  //   v: 0,
  //   w: 0
  // }, {
  ...
  // const fw_drawStyle = [{v:0,w:0},{v:1,w:0},{v:2,w:100},{v:2,w:0},{v:3,w:0}];  //debug
  ...
  if (random() > 1.8 && featureText_drawStyle != "Slashes" &&
    featureText_drawStyle != "Squiggles" && 
    featureText_drawStyle != "Beta"
    ) {
  ...
  // }];
  const fw_outlines = [{v:0,w:0},{v:1,w:33},{v:2,w:0}]; //debug outlines
  ...
  const fw_outlineStyle = [{
    ...
    v: 1,
    w: featureText_drawStyle == "Beta" ? 85 : 0
  }];
  ...
  // const fw_backgroundType = [{v:0,w:5},{v:1,w:5}];//debug
  // f_backgroundType = rarityPicker(fw_backgroundType, random());
  // if (f_backgroundType == 1) {
  //   featureText_backgroundType = "Light"
  // } else {
  //   featureText_backgroundType = "Dark"
  // }
  ...
  // }, {
  //   v: 2,
  //   w: featureText_drawStyle == "Beta2" ? 36 : 13
  // }, {
  //   v: 3,
  //   w: featureText_drawStyle == "Beta2" ? 20 : 16
  // }, {
  //   v: 4,
  //   w: 0
  // }];
  ...
  //   w: 14
  // }];
  const fw_structureType = [{v:0,w:1}]; //debug cube
  ...
  const fw_multigrid = [{
    v: 0,
    w: featureText_drawStyle == "Beta" ? 20 : 80
  }, {
    v: 1,
    w: 20
  }, {
    v: 2,
    w: featureText_drawStyle == "Beta" ? 40 : 0
  }];
  f_multigrid = rarityPicker(fw_multigrid, random());
  if (f_multigrid == 0 || featureText_plopType == "Perfects" || featureText_plopType == "Spiral") {
...
handleUrlParams()
print(seedValue)
if (generatorMode != 1 && enableParams == 1) {handleFXParams()}
...
function setupBuffers(_restart) {
  ...
  if (_restart !== undefined && _restart == true) {
    ...
    gridBuffer.remove()
    progressBuffer.remove()
    bgBuffer.remove()
    perfectBuffer.remove()
    accentBuffer.remove()
  ...
  progressBuffer = createGraphics(outputWidth, outputHeight);
  bgBuffer = createGraphics(outputWidth, outputHeight);
  perfectBuffer = createGraphics(outputWidth, outputHeight);
  accentBuffer = createGraphics(outputWidth, outputHeight);
  ...
  // var newBuffer = createGraphics(outputWidth, outputHeight);
  // mainBuffer = newBuffer
  // featureBuffer = newBuffer
  // qubesBuffer = newBuffer
  // outlinesBuffer = newBuffer
  // gridBuffer = newBuffer
  // progressBuffer = newBuffer
  // bgBuffer = newBuffer
  // perfectBuffer = newBuffer
  ...
  progressBuffer.pixelDensity(1);
  bgBuffer.pixelDensity(1);
  perfectBuffer.pixelDensity(1);
  accentBuffer.pixelDensity(1);
  ...
  progressBuffer.colorMode(HSB, 255);
  bgBuffer.colorMode(HSB, 255);
  perfectBuffer.colorMode(HSB, 255);
  accentBuffer.colorMode(HSB, 255);
...
function restart(_perfect, _scale = 1, _zoom) {
  console.clear()
  resetVars();
  if (generatorMode != 1 && enableParams == 1) {handleFXParams()}
...
function buildGrids() {
  ...
  let breakCount = 0
  // while(faceCount <= 0 && breakCount < 1000) {
  //   breakCount++
  ...
  for (let i = 0; i < (featureText_plopType == "Perfect" ? 1 : gridCount) && breakCount < 1000; i++) {
    ...
    // cc = featureText_plopType == "Perfect" ? floor(random(9, 30)) : ceil(random(5.01, 18 / horizontalGridCount));
    let minCols = 5.01    
    ...
    if (1==1) {
      ...
      faceCount += g.qubeCount * 2

    } else {
      // print("qubeCount", g.qubeCount, (featureText_multigrid == false ? 3 : 0))
      // print("grid face area to grid area", g.faceArea/g.gridArea, (featureText_multigrid == false ? 0.12 : 0.12 / gridCount))
      // print("Discard Grid")
      i--
    }
    breakCount++
    if (breakCount > 900) {
      // print("breakcount=" + breakCount, faceCount)
      // print(showFace1,showFace2,showFace3,showFace4,showFace5,showFace6)
    }
...
function setupColors() {
  ...
  //this shit isn't seeming rand....so trying to really see if I'm losing my mind.
  // let randomValue = floor(random(qubeColors.length) - 0.0001)
  // let randomValue2 = random(10)
...
function setupCanvas(_resize, _scale = 1) {
  ...
  if (_resize == undefined || _resize == false) {
    mainCanvas = createCanvas(outputWidth, outputHeight);
  } else {
    mainCanvas.resize(outputWidth, outputHeight);
  }
...
function fineTuneFeatures() {
  ...
  if (random() > (featureText_drawStyle == "Squiggles" ? 0.85 : 0.5)) {
    colorEffect = "solid"
  } else {
  ...
  switch (featureText_continuity) {
    ...
    case "Explosive":
      ...
      break;
    default:
      incOverlapRate = random(0.0, 0.75)
  }

  if (random() > 0.3 && featureText_multigrid != false) {
    percStepType = "random"
  } else {
...
//debug remove this whole section in favor of the next line?
  ...
  if (featureText_drawStyle == "Slashes") {
    if (featureText_qubeType == "Stripesxxxx") {
      ...
      defaultPercStepFactor = random(0.01, 0.04)
    }
  } else if (featureText_drawStyle == "Squiggles") {
    defaultPercStepFactor = random(0.04, 0.08)
  } else {
    defaultPercStepFactor = featureText_chunky == true ? random(0.10, 0.14) : random(0.01, 0.035)
  }
  ...
  switch (featureText_humanDrawErrorFactor) {
    case "Skilled":
      humanDrawErrorFactor = (PI / random(25, 40))
      maxDrawSegment = random(0.02, 0.05)
      break;
    case "Amateur":
      humanDrawErrorFactor = (PI / random(10, 20))
      maxDrawSegment = random(0.03, 0.075)
      break;
    case "Wild":
      humanDrawErrorFactor = (PI / random(7, 9))
      maxDrawSegment = random(0.03, 0.075)
      break;
    default:
      humanDrawErrorFactor = 0
  ...
  customDrawSegmentLimits_outlines = {
    min: featureText_outlineStyle == "Perfect" || featureText_drawStyle == "Perfect" ? 0.5 * canvasMinSize : 0.01 * canvasMinSize,
    max: featureText_outlineStyle == "Perfect" || featureText_drawStyle == "Perfect" ? 0.5 * canvasMinSize : maxDrawSegment * canvasMinSize
  }

  strokeSize = canvasFactor * random(1, 1) * (map(humanDrawErrorFactor, (PI / 40), PI / 8, 1.5, 0.75, true))

  // print("customDrawSegmentLimits: ")
  // print( customDrawSegmentLimits)
  // print("strokeSize/canvasFactor = " + strokeSize/canvasFactor)
  // print("humanDrawErrorFactor = " + humanDrawErrorFactor)
  ...
  // print("colorSplitType fadeType", colorSplitType, fadeType)
...
function drawBackground() {
  ...
  //   bgBuffer.strokeWeight(15)
  //   bgBuffer.stroke(100,5);
  //   bgBuffer.noFill()
  //   bgBuffer.rect(outputWidth/2, outputHeight/2, random(outputWidth/2, outputWidth), random(outputHeight/2, outputHeight))
  //   bgBuffer.pop()
  // }
  ...
  if (frameCount == debugFrame && printDebugs == 1) {
    print(debugLineCt + " frame 1: " + random().toFixed(2));
    debugLineCt++
  }
  let frameStrokeSize = strokeSize * 2

  let sColors
  sColors = color(random(chosenColors.colors))
  ...
  //debug, we don't need these.  I only kept them for testing
  // squiggleXDirectionBG = random((0, 0.005)) * random([-1, 1])
  // squiggleYDirectionBG = random((0, 0.005)) * random([-1, 1])
  ...
  if (frameCount == debugFrame && printDebugs == 1) {
    print(debugLineCt + " drawBackground END: " + random().toFixed(2));
    debugLineCt++
  }
...
function buildQubes(_grid) {
  ...
  for (let thisPoint of _grid.bigPoints) {
    ...
    if (qube.faceCount > 0) {
      ...
      let qubeSquiggleSegmentCount = ceil(map(averageFaceAreaToCanvasRatio, 0.001, 0.05, 1.5, 7, true))
      let qubeSquiggleDensity = map(averageFaceAreaToCanvasRatio, 0.001, 0.04, 0.1, 0.15, true)

      // if (qubeStrokeSize / (_grid.faceArea / _grid.qubeCount) * 10000 > 2) {
      //   qubeStrokeSize = random(0.5, 2) / 10000 * (_grid.faceArea / _grid.qubeCount)
      // }
...
function showQubes() {
  if (frameCount == debugFrame && printDebugs == 1) {
    print(debugLineCt + " showQubes 1: " + random().toFixed(2));
    debugLineCt++
  }
  ...
  if (frameCount == debugFrame && printDebugs == 1) {
    print(debugLineCt + " showQubes 2: " + random().toFixed(2));
    debugLineCt++
  }
...
function drawAccent() {
  for (let qube of qubes) {
    for (let face of qube.faces) {
      let xoff = 0
      // for (let xoff = -0.05; xoff <= 0.05; xoff+=0.01) {
      // drawOutlines(accentBuffer, face.v1, face.v2, face.v3, "#000000", 1, 1, xoff)
      ...
      c.setAlpha(100)
      accentBuffer.fill(c)
      accentBuffer.strokeWeight(strokeSize)
      accentBuffer.noStroke()
      // accentBuffer.stroke("#000000")
      // accentBuffer.beginShape()
      // accentBuffer.vertex(face.v1.x + (face.v1.x * xoff), face.v1.y + (face.v1.y * xoff))
      // accentBuffer.vertex(face.v2.x + (face.v2.x * xoff), face.v2.y + (face.v2.y * xoff))
      // accentBuffer.vertex(face.v3.x + (face.v3.x * xoff), face.v3.y + (face.v3.y * xoff))
      // accentBuffer.endShape()

      // xoff = -0.05
      // yoff = -0.05
      // accentBuffer.beginShape()
      // accentBuffer.vertex(face.v1.x + (face.v1.x * xoff), face.v1.y + (face.v1.y * yoff))
      // accentBuffer.vertex(face.v2.x + (face.v2.x * xoff), face.v2.y + (face.v2.y * yoff))
      // accentBuffer.vertex(face.v3.x + (face.v3.x * xoff), face.v3.y + (face.v3.y * yoff))
      // accentBuffer.endShape()
      // xoff = 0.05
      // yoff = -0.05
      // accentBuffer.beginShape()
      ...
      let point1 = p5.Vector.lerp(face.v1, face.v3, -0.1)
      let point2 = p5.Vector.lerp(face.v2, p5.Vector.lerp(face.v1, face.v2, 0.5), -0.1)
      let point3 = p5.Vector.lerp(face.v1, face.v3, 1.1)
      accentBuffer.beginShape()
      accentBuffer.vertex(point1.x, point1.y)
      accentBuffer.vertex(point2.x, point2.y)
      accentBuffer.vertex(point3.x, point3.y)
      accentBuffer.endShape()


      // accentBuffer.line(face.v1.x, face.v1.y, face.v2.x, face.v2.y)
      // accentBuffer.line(face.v2.x, face.v2.y, face.v3.x, face.v3.y)
      accentBuffer.pop()
...
// HELD_nextDrawnPerc, showQubesPercInc, undefined, this.qubeOriginalScaleArea, this.squiggleSegmentCount, HELD_faceNextPercDraw)
function drawOutlines(gb, _v1, _v2, _v3, _color, _perc, _percInc, _offset, _qubeOriginalScaleArea, _squiggleSegmentCount, _HELD_faceNextPercDraw, _percStep, _faceEndPerc, _squiggleDensity) {
  ...
  if (frameCount == debugFrame && printDebugs == 1) {
    print(debugLineCt + " drawOutlines 1: " + random().toFixed(2));
    debugLineCt++
  }
  _perc = round(_perc, 8)
  ...
  for (let perc = _HELD_faceNextPercDraw; round(perc, 8) <= max(percEnd, _HELD_faceNextPercDraw); perc += percStep) {
    ...
    // let endVector1 = p5.Vector.lerp(startVector1, _v1, faceEndPerc)
    // let endVector2 = p5.Vector.lerp(startVector2, _v3, faceEndPerc)
    ...
    startVector2.x += offset * startVector1.x
    startVector2.y += offset * startVector1.y
    endVector1.x += offset * startVector1.x
    endVector1.y += offset * startVector1.y
    endVector2.x += offset * startVector1.x
    endVector2.y += offset * startVector1.y
    ...
    if (frameCount == debugFrame && printDebugs == 1) {
      print(debugLineCt + " drawOutlines 2: " + random().toFixed(2));
      debugLineCt++
    }
    if (featureText_drawStyle == "Human" && featureText_outlineStyle == "Match") {
      customLine(gb, startVector1, endVector1, doffInc, customDrawSegmentLimits_outlines, _color, defaultOutlinesAlpha, strokeSize, outputWidth, outputHeight, "hash", humanDrawErrorFactor * 0.5, incOverlapRate, true, colorEffect)
      customLine(gb, startVector2, endVector2, doffInc, customDrawSegmentLimits_outlines, _color, defaultOutlinesAlpha, strokeSize, outputWidth, outputHeight, "hash", humanDrawErrorFactor * 0.5, incOverlapRate, true, colorEffect)
    } else if (featureText_drawStyle == "Squiggles" && featureText_outlineStyle == "Match") {
      // customLinebySquiggles(gb, startVector1, endVector1, doffInc, customDrawSegmentLimits_outlines, _color, defaultOutlinesAlpha, strokeSize, outputWidth, outputHeight, "perfect")
      // customLinebySquiggles(gb, startVector2, endVector2, doffInc, customDrawSegmentLimits_outlines, _color, defaultOutlinesAlpha, strokeSize, outputWidth, outputHeight, "perfect")
      ...
      customLine(gb, startVector1, endVector1, doffInc, customDrawSegmentLimits_outlines, _color, defaultOutlinesAlpha, strokeSize, outputWidth, outputHeight, "perfect", undefined, undefined, false, "solid")
      customLine(gb, startVector2, endVector2, doffInc, customDrawSegmentLimits_outlines, _color, defaultOutlinesAlpha, strokeSize, outputWidth, outputHeight, "perfect", undefined, undefined, false, "solid")
      // customLine(gb, startVector1, endVector1, doffInc, customDrawSegmentLimits_outlines, _color, defaultOutlinesAlpha, strokeSize, outputWidth, outputHeight, "hash", humanDrawErrorFactor * 0.3, incOverlapRate, true, "solid")
      // customLine(gb, startVector2, endVector2, doffInc, customDrawSegmentLimits_outlines, _color, defaultOutlinesAlpha, strokeSize, outputWidth, outputHeight, "hash", humanDrawErrorFactor * 0.3, incOverlapRate, true, "solid")
    }
  }

  //outline dots
  // let cSize = min(0.0015 * _qubeOriginalScaleArea, 0.005 * sqrt(canvasArea))
  // let noiseFrequency
  // let noiseAmp
  // let density
  // if (featureText_drawStyle == "Squiggles") {
  ...
  //   if (_v1.x == 1600.0690936104027 && _v1.y == 1315.4487016046837) {print("DOTS", _v1.x, _v1.y,)}
    
  //   if (featureText_drawStyle == "Perfect" || featureText_outlineStyle == "Perfect" || featureText_drawStyle == "Beta") {
  //     // let oc = outlineColor
  //     // oc.setAlpha(255)
  //     gb.push()
  //     gb.fill(_color)
  ...
  if (frameCount == debugFrame && printDebugs == 1) {
    print(debugLineCt + " drawOutlines 3 (" + frameCount + "): " + random().toFixed(2));
    debugLineCt++
  }
  if (frameCount == debugFrame && printDebugs == 1) {
    // print((startVector1.x / outputWidth).toFixed(2), (startVector1.y / outputHeight).toFixed(2), (startVector2.x / outputWidth).toFixed(2), (startVector2.y / outputHeight).toFixed(2))
    debugLineCt++
  }
  // }
}
...
function showFeatures(gb) {
  ...
  gb.text(
    ...
    "Color Selection Type = " + featureText_colorPickerType + "\n" +
    "Structure Type = " + featureText_structureType + "\n" +
    "Touches Len = " + touches.length, canvasMinSize * 0.025, canvasMinSize * 0.063)
...
function addGrain(gb) {
  // set the color of the image buffer to white
  // gb.background(255);

  // load the pixels from the image buffer
  // gb.loadPixels();

  //   // loop through every pixel in the pixel array
  //   for (let i = 0; i < gb.pixels.length; i += 4) {
  //     // get a random grayscale value between 0 and 255
  //     let randomGray = random(35, 185);

  //     // set the red, green, and blue values of the pixel to the random grayscale value
  //     gb.pixels[i] = randomGray;
  //     gb.pixels[i+1] = randomGray;
  //     gb.pixels[i+2] = randomGray;

  //     // reduce the alpha value of the pixel by 50%
  //     gb.pixels[i+3] = map(gb.pixels[i+3], 0, 255, 0, 20);
  //   }

  //   // update the image buffer with the modified pixel values
  //   gb.updatePixels();
  ...
  //   let scale = 2
  //   let ix = (i + 1) % 3
  //   let py = floor(floor(ix / outputWidth) / scale)
  //   let px = ix - (py * (outputWidth / scale))
  ...
  //   // reduce the alpha value of the pixel by 50%
  //   gb.pixels[i+3] = map(gb.pixels[i+3], 0, 255, 0, 255);
  // }
...
function draw() {
  // if (touchStart > 0 && millis() - touchStart >= touchDuration && exportFlag == 0) {
  //   let scale = 2
  //   touchStart = 0
  //   exportFlag = 1
  //   restart(false, scale)
  ...
  if (frameCount == debugFrame && printDebugs == 1) {
    print("Draw Start (" + frameCount + "): " + random().toFixed(2))
    debugLineCt++
  }
  // clear()
  if (frameCount == debugFrame && printDebugs == 1) {
    print(debugLineCt + " draw 1: " + random().toFixed(2));
    debugLineCt++
  }
  if (!tokenSaved) {
    showQubes();
  ...
  if (frameCount == debugFrame && printDebugs == 1) {
    print(debugLineCt + " draw 11: " + random().toFixed(2));
    debugLineCt++
  }
  if (switchBackground == 1) {
    mainBuffer.image(bgBuffer, 0 , 0)
  }
  if (switchShowGrain == 1) {
    mainBuffer.push()
  ...
  if (frameCount == debugFrame && printDebugs == 1) {
    print(debugLineCt + " draw 12: " + random().toFixed(2));
    debugLineCt++
  }
  ...
  // if (switchShowAccent == 1) {
  // push()
  //   let tintFactor = map(faceCount, 1, 50, 0.35, 0.15, true)
  //   tint(255, 255 * tintFactor)
  //   image(accentBuffer, 0, 0)
  // pop()
  // }

  // if (1 == 2 && tokenSaved) {
  //   push()
  //   // image(qubesBuffer, outputWidth * randomSV(0.01, 0.05), outputHeight * randomSV(0.01, 0.05))
  //   // image(qubesBuffer, outputWidth * randomSV(0.01, 0.05), outputHeight * randomSV(0.01, 0.05))
  //   // image(qubesBuffer, outputWidth * randomSV(0.01, 0.05), outputHeight * randomSV(0.01, 0.05))

  //   tint(255, 70)
  //   image(lowlightBuffer, outputWidth * 0.03 * -1, outputHeight * 0.03 * -1)
  //   image(highlightBuffer, outputWidth * 0.03 * -1, outputHeight * 0.03 * -1)

  //   image(lowlightBuffer, outputWidth * 0.03 * -1, outputHeight * 0.03)
  //   image(highlightBuffer, outputWidth * 0.03 * -1, outputHeight * 0.03)

  //   image(lowlightBuffer, outputWidth * 0.03, outputHeight * 0.03 * -1)
  //   image(highlightBuffer, outputWidth * 0.03, outputHeight * 0.03 * -1)

  //   image(lowlightBuffer, outputWidth * 0.03, outputHeight * 0.03)
  //   image(highlightBuffer, outputWidth * 0.03, outputHeight * 0.03)

  //   tint(255, 50)
  //   image(lowlightBuffer, outputWidth * 0.06 * -1, 0)
  //   image(highlightBuffer, outputWidth * 0.06 * -1, 0)

  //   image(lowlightBuffer, outputWidth * 0.06, 0)
  //   image(highlightBuffer, outputWidth * 0.06, 0)

  //   image(lowlightBuffer, 0, outputHeight * 0.06 * -1)
  //   image(highlightBuffer, 0, outputHeight * 0.06 * -1)

  //   image(lowlightBuffer, 0, outputHeight * 0.06)
  //   image(highlightBuffer, 0, outputHeight * 0.06)

  //   tint(255, 30)
  //   image(lowlightBuffer, outputWidth * 0.03 * -1, outputHeight * 0.06 * -1)
  //   image(highlightBuffer, outputWidth * 0.03 * -1, outputHeight * 0.06 * -1)

  //   image(lowlightBuffer, outputWidth * 0.03 * -1, outputHeight * 0.06)
  //   image(highlightBuffer, outputWidth * 0.03 * -1, outputHeight * 0.06)

  //   image(lowlightBuffer, outputWidth * 0.03, outputHeight * 0.06 * -1)
  //   image(highlightBuffer, outputWidth * 0.03, outputHeight * 0.06 * -1)

  //   image(lowlightBuffer, outputWidth * 0.03, outputHeight * 0.06)
  //   image(highlightBuffer, outputWidth * 0.03, outputHeight * 0.06)
  ...
  //   let randomx = randomSV(0.01, 0.03) 
  //   let randomy = randomSV(0.01, 0.03)

  //   tint(255, 85)
  //   image(lowlightBuffer, outputWidth * random(randomx * 0.8, randomx * 1.2) * 1, outputHeight * random(randomy * 0.8, randomy * 1.2) * -1.1)
  //   image(highlightBuffer, outputWidth * random(randomx * 0.8, randomx * 1.2) * 1, outputHeight * random(randomy * 0.8, randomy * 1.2) * -1.1)
  //   tint(255, 70)
  //   image(lowlightBuffer, outputWidth * random(randomx * 0.8, randomx * 1.2) * 2, outputHeight * random(randomy * 0.8, randomy * 1.2) * -2.1)
  //   image(highlightBuffer, outputWidth * random(randomx * 0.8, randomx * 1.2) * 2, outputHeight * random(randomy * 0.8, randomy * 1.2) * -2.1)
  //   tint(255, 45)
  //   image(lowlightBuffer, outputWidth * random(randomx * 0.8, randomx * 1.2) * 3, outputHeight * random(randomy * 0.8, randomy * 1.2) * -3.1)
  //   image(highlightBuffer, outputWidth * random(randomx * 0.8, randomx * 1.2) * 3, outputHeight * random(randomy * 0.8, randomy * 1.2) * -3.1)
  //   tint(255, 30)
  //   image(lowlightBuffer, outputWidth * random(randomx * 0.8, randomx * 1.2) * 4, outputHeight * random(randomy * 0.8, randomy * 1.2) * -4.1)
  //   image(highlightBuffer, outputWidth * random(randomx * 0.8, randomx * 1.2) * 4, outputHeight * random(randomy * 0.8, randomy * 1.2) * -4.1)
  ...
  if (frameCount == debugFrame && printDebugs == 1) {
    print(debugLineCt + " draw 2: " + random().toFixed(2));
    debugLineCt++
  }

  let maxAlpha = random(150, 220)
  if (featureText_echo == "Growth" && switchEcho == 1) {
    for (i = 1; i <= 4; i++) {
      mainBuffer.push()
      mainBuffer.tint(255, maxAlpha / pow(i, 2))
      let echoLowImage = createGraphics(outputWidth * (1 + (pow(i, 1.2) * 0.1)), outputHeight * (1 + (pow(i, 1.2) * 0.1)))
      let echoHighImage = createGraphics(outputWidth * (1 + (pow(i, 1.2) * 0.1)), outputHeight * (1 + (pow(i, 1.2) * 0.1)))
      echoLowImage.image(lowlightBuffer, 0, 0, echoLowImage.width, echoLowImage.height)
      echoHighImage.image(highlightBuffer, 0, 0, echoHighImage.width, echoHighImage.height)
      mainBuffer.imageMode(CENTER)
      mainBuffer.blendMode(MULTIPLY)
      mainBuffer.image(echoLowImage, outputWidth / 2, outputHeight / 2)
      mainBuffer.image(echoHighImage, outputWidth / 2, outputHeight / 2)
      mainBuffer.pop()
    }
  }

  if (featureText_echo == "Directional" && switchEcho == 1) {
    for (i = 1; i <= 4; i++) {
      mainBuffer.push()
      mainBuffer.tint(255, maxAlpha / pow(i, 2))
      mainBuffer.imageMode(CENTER)
      mainBuffer.rotate(PI * 0.02 * pow(i, 2))
      mainBuffer.imageMode(CORNER)
      mainBuffer.blendMode(MULTIPLY)
      // let echoLowImage = createGraphics(outputWidth * (1 + (pow(i, 1.2) * 0.1)), outputHeight * (1 + (pow(i, 1.2) * 0.1)))
      // let echoHighImage = createGraphics(outputWidth * (1 + (pow(i, 1.2) * 0.1)), outputHeight * (1 + (pow(i, 1.2) * 0.1)))
      // echoLowImage.image(lowlightBuffer, 0, 0, echoLowImage.width, echoLowImage.height)
      // echoHighImage.image(highlightBuffer, 0, 0, echoHighImage.width, echoHighImage.height)
      // imageMode(CENTER)
      mainBuffer.image(lowlightBuffer, outputWidth * (i * 0.05) * echoXDirection, outputHeight * (i * 0.05) * echoYDirection)
      mainBuffer.image(highlightBuffer, outputWidth * (i * 0.05) * echoXDirection, outputHeight * (i * 0.05) * echoYDirection)
      mainBuffer.pop()
    }
  }

  if (frameCount == debugFrame && printDebugs == 1) {
    print(debugLineCt + " draw 3: " + random().toFixed(2));
    debugLineCt++
  }

  // if(tokenSaved) {filter(BLUR, 3.5)}
  ...
  if (frameCount == debugFrame && printDebugs == 1) {
    print(debugLineCt + " draw 4 (" + frameCount + "): " + random().toFixed(2));
    debugLineCt++
  }
  if (tokenSaved && !(touchStart > 0)) {
    if (exportFlag == 1 && tokenSaved == 1) {
      // print("exporting after touch")
      exportFlag = 0
      exportCanvas(outputWidth, outputHeight)
    }
    // for (i=0; i<=20; i+=5) {
    //   image(outlinesBuffer, i, i)
    //   image(qubesBuffer, i, i)
    // }
    noLoop();
    if (generatorMode && generationCount < maxGenerationCount) {
      generationCount++
      image(featureBuffer, 0, 0)
      save(mainBuffer, 'qube_' + featureText_color + "_" + colorEffect + "_" + seedValue + "_" + featureText_drawStyle +  "_" + featureText_plopType + '.png');
...
function keyPressed() {
  ...
  //remove
  if (keyCode == 68 && devKeyCodesEnabled)  { // D
    switchShowGrid *= -1
    gridBuffer.clear()
    showGrid(gridBuffer)
    loop();
  ...
  //remove
  if (keyCode == 78 && devKeyCodesEnabled) { // N
    print("Next!")
    seedValue += 1
    restart();
    // if (generatorMode) {  
  ...
  //remove
  if (keyCode == 76 && devKeyCodesEnabled) { // L
    print("Last!")
    seedValue += -1
    restart();
    // if (generatorMode) {  
    ...
    for (let newFace of newFaces) {
      if (1==1) {
        ...
        if (!(v1.dist(v2) * v1.dist(v3) * v2.dist(v3) * v4.dist(v5) * v4.dist(v6) * v5.dist(v6) == 0) &&
          newFace.showFace == 1) {
          let face1 = new Face(v1, v2, v3, colors, newFace.style, newFace.colorSplitType, newFace.showFace, newFace.showOutlines, percStep, betaColor, flipColors)
          let face2 = new Face(v4, v5, v6, colors, newFace.style, newFace.colorSplitType, newFace.showFace, newFace.showOutlines, percStep, betaColor, flipColors)
...
class Face {
  constructor(_v1, _v2, _v3, _colors, _style, _colorSplitType, _showFace, _showOutlines, _percStep, _betaColor, _flipColors) {
    this.v1 = _v1
    this.v2 = _v2
    this.v3 = _v3
    this.colors = _colors
  ...
  show() {
    ...
    if (this.showFace) {
      ...
      if (frameCount == debugFrame && printDebugs == 1) {
        print(debugLineCt + " showFace 1: " + random().toFixed(2), this.v1.x / canvasMinSize, this.v2.x / canvasMinSize, this.v3.x / canvasMinSize, nextDrawnPerc, showQubesPercInc, this.percStep);
        debugLineCt++
    ...
    if (frameCount == debugFrame && printDebugs == 1) {
      print(debugLineCt + " showFace 2: " + random().toFixed(2));
      debugLineCt++
    }
    ...
    if (frameCount == debugFrame && printDebugs == 1) {
      print(debugLineCt + " showFace END: " + random().toFixed(2));
      debugLineCt++
    }
  ...
  betaFaceDraw(v1, v2, v3, _colors, _drawType, _perc, _percInc, _colorSplitType, _percStep, _faceArea, _HELD_faceNextPercDraw) {
    ...
    // if (round(_perc, 2) >= 0.95) {
      ...
      // for(let i = 0; i<=1; i = round((i + inc), 8)) {
        //  let c = color(_color)

        // original beta style
        // let startVector = p5.Vector.lerp(v2, v1, i)
        ...
        if (i >= 1) {c.setAlpha(alpha * 0.5)}
        // if (round(this.v1.x, 3) == 257.706 && round(this.v2.y, 4) == 154.8857) {print(i, c, size)}
        ...
        // qubesBuffer.noStroke()
        // qubesBuffer.ellipse(plotVector.x, plotVector.y, 2, 2)
        // qubesBuffer.square(plotVector.x, plotVector.y, size, size)
  ...
  customFaceDraw(v1, v2, v3, _colors, _drawType, _perc, _percInc, _colorSplitType, _percStep) {
    ...
    let doWeFlip
    // if (random() > 0.37) {doWeFlip = true}
    if (this.doWeFlip && _perc >= flipPlace) {doWeFlip = true}
    ...
    if (_drawType == "Crayon") {
      ...
      for (let perc = this.nextPercDrawn; round(perc, 8) <= max(percEnd, this.nextPercDrawn); perc += percStep) {
        ...
        let alpha = map(round(perc * 100) % fadeRate, minFadeRate, maxFadeRate, minAlpha, maxAlpha) * 1.5
        if (_colors.length == 1) {
          c = color(_colors[0])
        } else {
        ...
        if (featureText_drawStyle == "Human") {
          ...
          perfectBuffer.push()
          perfectBuffer.noFill()
          perfectBuffer.strokeWeight(this.faceStrokeSize * strokeSizePercStepModifier)
          perfectBuffer.stroke(c)
          perfectBuffer.line(startVector.x, startVector.y, endVector.x, endVector.y)
          perfectBuffer.pop()
        }
        ...
        // this.showHighlights(highlightTypeTracker == 1 ? highlightBuffer : lowlightBuffer, startVector, highlightEndVector, highlightTypeTracker == 1 ? 0 : 255, this.faceStrokeSize, highlightAlpha)

        //   highlightBuffer.push()
        //   highlightBuffer.noFill()
        //   highlightBuffer.strokeWeight(this.faceStrokeSize * 1)
        //   highlightBuffer.stroke(highlightTypeTracker == 1 ? 0 : 255, highlightAlpha)
        //   highlightTypeTracker *= -1
        //   highlightBuffer.line(startVector.x, startVector.y, highlightEndVector.x, highlightEndVector.y)
      ...
      for (let perc = this.nextPercDrawn; round(perc, 8) <= max(percEnd, this.nextPercDrawn); perc += percStep) {
        ...
        let alpha = map(round(perc * 100) % fadeRate, minFadeRate, maxFadeRate, minAlpha, maxAlpha)
        //  let previousPlotPoint1
        //  let previousPlotPoint2
        ...
        if (featureText_drawStyle == "Human") {
          //    let perc2Step = {
          //      min: 0.01 * canvasMinSize,
          //      max: 0.02 * canvasMinSize
          //    }
      ...
      if (frameCount == debugFrame && printDebugs == 1) {
        print(debugLineCt + " customFaceDraw preloop (" + frameCount + "): " + random().toFixed(2), this.nextPercDrawn, percStep, percEnd)
      }

      for (let perc = this.nextPercDrawn; round(perc, 8) <= max(percEnd, this.nextPercDrawn); perc += percStep) {
        if (frameCount == debugFrame && printDebugs == 1) {
          print(debugLineCt + " customFaceDraw loop 1 (" + frameCount + "): " + random().toFixed(2), perc, percStep, percEnd)
        }
        ...
        if (featureText_drawStyle == "Human") {
          ...
          customLine(qubesBuffer, startVector, midVector, drawConsistency, customDrawSegmentLimits, c, alpha, this.faceStrokeSize * strokeSizePercStepModifier, outputWidth, outputHeight, "hash", humanDrawErrorFactor, incOverlapRate, true, colorEffect)
        ...
        // let lineHeading1 = (p5.Vector.sub(startVector, midVector).heading()) - (abs((p5.Vector.sub(startVector, midVector).heading())) > (PI/2) ? PI : 0)
        // let lineHeading2 = (p5.Vector.sub(midVector, endVector).heading()) - (abs((p5.Vector.sub(midVector, endVector).heading())) > (PI/2) ? PI : 0)
        ...
        highlightTypeTracker *= -1
        // this.showHighlights(highlightTypeTracker == 1 ? highlightBuffer : lowlightBuffer, highlightStartVector, highlightMidVector, highlightTypeTracker == 1 ? 0 : 255, this.faceStrokeSize, highlightAlpha)
        // this.showHighlights(highlightTypeTracker == 1 ? highlightBuffer : lowlightBuffer, highlightMidVector, highlightEndVector, highlightTypeTracker == 1 ? 0 : 255, this.faceStrokeSize, highlightAlpha)
        ...
        //   highlightBuffer.stroke(highlightTypeTracker == 1 ? 0 : 255, highlightAlpha)
        //   highlightTypeTracker *= -1
        //   highlightBuffer.line(highlightStartVector.x, highlightStartVector.y, highlightMidVector.x, highlightMidVector.y)
        //   highlightBuffer.line(highlightMidVector.x, highlightMidVector.y, highlightEndVector.x, highlightEndVector.y)
        //  //  customLinebyCrosses(qubesBuffer, plotPoint, endVector, drawConsistency, customDrawSegmentLimits, c, alpha, this.faceStrokeSize * 1, outputWidth, outputHeight, "perfect", humanDrawErrorFactor, spaceBetweenPercStep/3)
        //  highlightBuffer.pop()
      ...
      if (frameCount == debugFrame && printDebugs == 1) {
        print(debugLineCt + " customFaceDraw post loop (" + frameCount + "): " + random().toFixed(2), this.nextPercDrawn, percStep);
        debugLineCt++;
      }
      ...
      for (let perc = this.nextPercDrawn; round(perc, 8) <= max(percEnd, this.nextPercDrawn); perc += percStep) {
        ...
        if (featureText_drawStyle == "Human") {
          ...
          customLinebyCrosses(qubesBuffer, plotPoint, endVector, drawConsistency, customDrawSegmentLimits, c, alpha * 1.5, this.faceStrokeSize * 1, outputWidth, outputHeight, "hash", humanDrawErrorFactor, slashSpacing, slashLength, colorEffect)
        } else if (featureText_drawStyle == "Squiggles") {
          c.setAlpha(alpha * 2)

          if (frameCount == debugFrame && printDebugs == 1) {
            print(debugLineCt + " customFaceDraw step pre-squiggles (" + frameCount + "): " + random().toFixed(2));
            debugLineCt++;
          }
          ...
          if (frameCount == debugFrame && printDebugs == 1) {
            print(debugLineCt + " customFaceDraw step post-squiggles (" + frameCount + "): " + random().toFixed(2));
            debugLineCt++;
          }
        } else if (featureText_drawStyle == "Beta") {
          let c = color(_colors[0])
        ...
        //  let highlightStartVector = p5.Vector.lerp(v2, v3, max(0, perc-.05))
        //  let highlightEndVector = p5.Vector.lerp(v1, v3, min(1, perc+.01))
        ...
        //  highlightBuffer.stroke(highlightTypeTracker == 1 ? 0 : 255, highlightAlpha)
        //  highlightTypeTracker *= -1
        //  highlightBuffer.line(highlightStartVector.x, highlightStartVector.y, highlightEndVector.x, highlightEndVector.y)
        // //  customLinebyCrosses(qubesBuffer, plotPoint, endVector, drawConsistency, customDrawSegmentLimits, c, alpha, this.faceStrokeSize * 1, outputWidth, outputHeight, "perfect", humanDrawErrorFactor, spaceBetweenPercStep/3)
        // highlightBuffer.pop()
...
function exportCanvas(bufferWidth, bufferHeight) {
  ...
  save(saveBuffer, title + "_" + fxhash + "_" + bufferWidth + "x" + bufferHeight + ".png")
...
function handleFXParams() {
  ...
  // print("featureText_canvasRatio: " + featureText_canvasRatio)
  ...
  // let fparam_plopType = $fx.getParam("fparam_plopType")
  // featureText_plopType = fparam_plopType
  // print("featureText_plopType: " + featureText_plopType)
  
  // switch (featureText_plopType) {
  //   case "Perfect":
  //     f_plopType = 0
  //     break;
  //   case "Grid":
  ...
  // print("featureText_frame: " + featureText_frame)
  ...
  // let fparam_outlines = $fx.getParam("fparam_outlines")
  // featureText_outlines = fparam_outlines
  // print("featureText_outlines: " + featureText_outlines)
  
  // switch (featureText_outlines) {
  //   case "None":
  //     f_outlines = 0
  //     break;
  //   case "All":
  ...
  // let fparam_outlineStyle = $fx.getParam("fparam_outlineStyle")
  // featureText_outlineStyle = fparam_outlineStyle
  // print("featureText_outlineStyle: " + featureText_outlineStyle)
  
  // switch (featureText_outlineStyle) {
  //   case "Match":
  //     f_outlineStyle = 0
  //     break;
  //   case "Perfect":
