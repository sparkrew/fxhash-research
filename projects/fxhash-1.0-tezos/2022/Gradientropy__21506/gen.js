...
function characterizeArray(a,msg,bLog=true){
...
function getIntersectingPoly(pts1,polys){
  for(let i=0;i<polys.length;i++){
    if(doPolysIntersect(pts1,polys[i]))
      return i
  }
  return -1
}
...
function doPolysIntersect(pts1,pts2){
  ...
  for(let i=0;i<a.length;i++){
    ...
    let {min: minA, max: maxA} = polyProjectInAxis(pts1,axis.x,axis.y)
    let {min: minB, max: maxB} = polyProjectInAxis(pts2,axis.x,axis.y)
...
function polyProjectInAxis(pts,x,y){
  o = {
    min:Infinity,
    max:-Infinity
  }
  for (let i = 0; i < pts.length; i++) {
    let px = pts[i].x
    let py = pts[i].y
    var proj = (px*x + py*y) / Math.sqrt(x*x + y*y)
...
function dilatePoints(pts,f,bX,bY,bAbs,aVar){
  f = f || 1
  bX = bX == null ? true : bX
  bY = bY == null ? true : bY
  ...
  for(let i=0;i<pts.length;i++){
    ...
    if(bAbs){
    	...
    	let vecUnit = vectorFrom({x:o.xm,y:o.ym},pts[i],true)
	    if(bX)
	    ...
	    if(bX)
	      pts2[i].x = (pts2[i].x - o.xm)*fv + o.xm
	    if(bY)
	      pts2[i].y = (pts2[i].y - o.ym)*fv + o.ym
    }
  }
  return pts2
}
...
function drawSimpleCurve(dc,p1,p2,fDeflection=.5,c){
  if(c){
    if(Array.isArray(c))
      c = rgbToString(c)
    dc.strokeStyle = c
...
function drawCurve(dc,pts,pStart,op,bClose=false,pEnd,bReverse=false){
...
function fillPoly(dc,pts,c,alpha,fvar,bCurvy=false){
  let pts0 = pts
  if(fvar != null){
...
function drawSphere(dc,x,y,r,r2,rot,c,alpha=1,xshift=0,yshift=0,highlightWhiteness=1){
  ...
  gradient.addColorStop(.7, rgbToString(c,alpha))
  gradient.addColorStop(.85, rgbToString(c,alpha))
  gradient.addColorStop(1, rgbToString(cD,.9*alpha))
...
function addNoiseToColor(c,n){
  c = c.slice()
  c[0] = constrain(c[0] + random(-n,n,'i'),0,255)
  c[1] = constrain(c[1] + random(-n,n,'i'),0,255)
  c[2] = constrain(c[2] + random(-n,n,'i'),0,255)
  return c
}
