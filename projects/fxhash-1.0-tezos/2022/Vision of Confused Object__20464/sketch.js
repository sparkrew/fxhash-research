let s;
let pallete = ["white","black","yellow", "orange", "violet", "purple", "red", "pink", "blue"];
let graphics;
let prevC = -10;
let ratio = 0.8;
function setup() {
  ...
  drawNoiseBackground(10, graphics);

  s = int(random(34, 10)) * 30;

  push();
  translate(width / 2, height / 2);
  rotate(int(random(8)) * 360/1);
...
function recursiveTriangles(arr, depth, max, rMax = width / 2) {
 ...
 circle(arr[target].x, arr[target].y, arr[(next + 1) % arr.length].x, arr[(next + 1) % arr.length].y, current.x, current.y);
  recursiveTriangles([arr[target], arr[(next + 1) % arr.length], current], depth + 1, max);
 circle(arr[next].x, arr[next].y, arr[(next + 1) % arr.length].x, arr[(next + 1) % arr.length].y, current.x, current.y);
