...
function makePoly(x,y,r,n,bClose){
	n = n || 3
	var pts = []
  for(let i=0;i<n;i++){
...
function fillTexturePoly(dc,pts,type,c,nStrength,angle,alpha,angleVar,alphaFill){
  ...
  let bSegments = type.indexOf('Segments') >= 0
  let o = processPoints(pts)
...
function hsvToCombination(h,s,v,t='splitcomplement'){
  // h in [0,360], s and v in [0,1]
...
function makeFlowField(nCols,nRows,seed,w,bZeroToOne,bWrap=false){
	// smaller w is noisier
  var grid = [];
...
function randomNormalRange(a0,a1,a2){
...
function cycle(t,v0,v1,nPeriods=1){
  let f = Math.sin(Math.PI*.5 + t*Math.PI*2*nPeriods)
  return map3(f,1,-1,1,v0,v1,v0)
}
