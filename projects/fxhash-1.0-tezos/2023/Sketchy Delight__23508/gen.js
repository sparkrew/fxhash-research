...
function processPoints(a,bCalcArea=false){
  ...
  o.r0 = distance(o.xa,o.ya,pn.x,pn.y)
  return o
}
...
function circlePackPoly(poly,frMax=.5,rMin=4,bCirclePoints=true,maxCircles=100){
  let a = []
  let o = processPoints(poly)
  let r = o.r0*frMax
  let maxTrials = 1000
...
function splitPoly(poly,res=.01,fDilate,bSimple=false,bDilateAbsolute=false,iSplitMode=0){
  let t1 = random()
  if(iSplitMode == 1){
    // split longest side
...
function createColorStopsForGradient(aClr,alpha,n,aWeights,nColorVary=0,ftVary=1,aStops0){
  ...
  for(let i=0; i < n; i++){
    ...
    if(aStops0 != null)
      t = aStops0[i].t
    let a = typeof alpha == 'function' ? alpha(i,n,t) : alpha
    let o ={
      t:t,
      cs:rgbToString(c,a),
      c:c,
...
function getPalette(palStart,palTrans,bUseExclusions=true){
  ...
  let aPal = [
    ...
    [[184, 11, 40],[5, 181, 250],[4, 50, 217],[95, 103, 135],[1, 3, 0]], // 5 red blue grey
...
function extendPointInField(field,x0,y0,lw,maxLen,aPoints,w,h,dSeg,bReturnNewPoints=false){
  let i,j,x,y,r,c;
  w = w || 1000
  h = h || 1000
  maxLen = maxLen || 100
...
function setValue(test,aVals,aRet,other){
...
function lerpPathDistance(f,a0){
  ...
  if(f > 1) f=1
  let a
  if(a0[0].sPerimeter == null || a0[0].sPerimeterFull == null)
    a = augmentPoints(a0)
  else
    a = a0
