class Grid {
  ...
  distortGrid() {
    ...
    if (featureText_distortion && switchDistortion == 1) {
      ...
      let distortedRows = [];
      let distortionTypeSelector = random([0, 1, 2, 3, 4, 0, 1, 2, 3]);
      ...
      let wellCount = gridCount == 1 ? random([1, 2, 2, 3, 3, 3, 4, 4, 5]) : random([1, 2])
      let wells = []
      for (let i = 0; i < wellCount; i++) {
        let wellCenter = createVector(
          this.startX + this.gridWidth * random(0.15, 0.85),
          this.startY + this.gridHeight * random(0.15, 0.85)
        );
        let wellFactor = (wellCount == 1 ? random(0.5, 0.8) : random(0.2, 0.8))
        let wellSize = canvasMinSize * wellFactor;
        let pushWell = {
  ...
  assignBigNodes() {
    ...
    if (featureText_plopType == "Organic") {
    ...
    } else if (featureText_plopType == "Perfect" && featureText_structureType == "Cube") {
      for (let thisPoint of this.points) {
        // for (let thisPoint of this.points) {
        if (
          floor(thisPoint.j + 1) % 3 == 0 && //if this row is ever third row starting with the second row
          //next two conditionals will stagger the columns that contain big nodes
          ((floor((thisPoint.j + 1) / 3) % 2 == 0 &&
              ((thisPoint.i + 1) / 1) % 2 == 1) || //and this is the 2nd group of 3 and this column is every 2nd
            (floor((thisPoint.j + 1) / 3) % 2 > 0 && (thisPoint.i + 1) % 2 == 0)) //or this is NOT the 2nd group of 3 and this column is not every 2nd
        ) {
          if (thisPoint.noise >= 0.0) {
            thisPoint.nodeType = "big1";
            this.bigPoints.push(thisPoint);
    ...
    } else if (featureText_plopType == "Frame") {
      let centerVector, circleMaxDist, circleMinDist
      ...
      this.circleMaxDist = circleMaxDist
      this.circleMinDist = circleMinDist
      let sizeFactor = random(0.1, 0.22)
      let plopXWidth = (sizeFactor * outputWidth)
      let plopYWidth = (sizeFactor * outputHeight)
      let edgeFactor = random(0.05, 0.2)
      let frontXEdge = outputWidth * edgeFactor
      let frontYEdge = outputHeight * edgeFactor
      let backXEdge = (outputWidth - frontXEdge)
      let backYEdge = (outputHeight - frontYEdge)

      for (let thisPoint of this.points) {
        let pointDistFromXSide = min(thisPoint.x - frontXEdge, backXEdge - thisPoint.x)
        let pointDistFromYSide = min(thisPoint.y - frontYEdge, backYEdge - thisPoint.y)

        if (
          (pointDistFromXSide < plopXWidth || pointDistFromYSide < plopYWidth) &&
          pointDistFromXSide > 0 && pointDistFromYSide > 0 &&
          (thisPoint.j + thisPoint.i) % 2 == 0
        ) {
          thisPoint.nodeType = "big1";
          this.bigPoints.push(thisPoint);
