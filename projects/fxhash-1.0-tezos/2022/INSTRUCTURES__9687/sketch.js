...
const { color, grid, parts } = manMachine;
...
function setupTraits() {
  ...
  let recipeArr = [{
    ...
    stepTxtArr: [`1)START(1)`, "   Place a cube bottom center."],
  }];
  ...
  for (let i = 0; i < instructionCount; i++) {
    ...
    if (cmd === "STACK") {
      ...
      stepTxtArr = [
        `   Pick a cube, note ${lockTxt} values. Place #'s `,
      ...
      stepTxtArr = [
        `   Pick a cube, note ${axis} value. Place #'s ${dirDisplay}`,
        `   or ${dir2Display} any # with same ${axis} value.`,
      ];
      instructionTxt = `${cmd}(${axis}`;
      factor = 6;
      if (axis === "x") {
  ...
  function logKey(e) {
    if (!e.repeat) { // Only once per keypress
      if (e.code === "KeyM") {
        ...
        draw();
      } else if ((e.code === "ArrowDown" || e.code === "KeyS") && !e.ctrlKey) {
        traits.vOffset = traits.vOffset < 9 ? traits.vOffset + 0.5 : traits.vOffset;
        decreaseFrameCount(1);
        draw();
