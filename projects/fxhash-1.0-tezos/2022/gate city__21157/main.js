...
let colorMain, colorMain2, palette;
  ...
  function setup() {
    ...
    randomSeed(randmax(0,1000));
  	...
  	changeweightDuringLine = 0;random([0,1]);
  	changeweightBeforeLine = 0;int(random(0.5,1.9));
  	changeweightbyhistory = 0;int(random(.9,1.7));
  	smallEvenChangesToLmWeight = 0;int(random(.5,1.5));
  	chSWbySC = 1;int(random(0.1,1.8));
  	//print(chSWbySC);
   ...
   // I think it looks better when maxsizechoice is small
  	...
  	minSizeChoice = 1;2;
  		//if make max size choice like 7 cool res
  	...
  	sizeChoice = 10;//int(random(5,maxSizeChoice+0.1)); // start at max, should get smaller
  	}else{
  		minSizeChoice = 3;
  	maxSizeChoice =min(5,max(windowSizeChoice,2));
  	sizeChoice=5;//int(random(2,maxSizeChoice+0.1));
  	}
  	scb = sizeChoice+2;
  	...
  	zzm = int(random(0,(windowHeight+windowWidth)/sf));
  	uum = int(random(0,(windowWidth+windowHeight)/sf));
  	nlim = 10000000000;random(minn,(windowWidth*windowHeight));
  	centerx = int(windowWidth/2);
  	//print(int(centerx));
  	centery = int(windowHeight/2);

  	addx = random(bufferx,centerx/1.3);
  	addy = random(buffery,centery/1.3);
  	//print(addx);
  	//print(addy);
  	x =centerx;int(random(bufferx,(windowWidth-bufferx)));
  	y =centery;int(random(buffery,(windowHeight-buffery)));
  	...
  	var mc = (c1+c2+c3)/3;
  		track=1;
  	bw = 0;//int(random(0,1.2));
  	if (bw==1){
  	c1=10;
  	c2=10;
  	c3=10;
  		track =int(random(0,1.3));
  		cc=random(0,2);
  	}

  	//l1 = int(random(0,1.2));
  	useaddx = 0;// int(random(0,1.2));
  	//print(useaddx);
  	//useaddx = 0;
  	...
  	blm = random(-0.1*mmult,(0.10+0.001)*mmult);
  		clm = 1; //random([1,3]);//random(-0.1*mmult,(0.10+0.001)*mmult);
  	}else{
  	alm = random(-10,10+0.001);
  	blm = random(-10,10+0.001);
  		clm = 1;//random([1,3]);random(-10,10+0.001);
  	}
  	//print(l1);
  	//print(l2);
  ...
  strokeWeight(sw);
          ...
          if(centerx+x+s<centerx*2-bufferx){
            ...
            if(centery+y+t<centery*2-buffery){
              ...
              if(centerx+x+s>bufferx){
                ...
                LenL=sqrt(sq((past_x +centerx)-(centerx+x+s)) + sq((centery+past_y) - (centery+y+t)));
                   ...
                   if(Len>=Longest){
                    ...
                    if(Len>=Longest){
                         ...
                         chosen_t=t;
  										...
  										Len = sqrt(sq((past_x +centerx)^clm*alm-(centerx+x+s)^clm*blm) + sq((centery+past_y)^clm*alm - (centery+y+t)^clm*blm))
  										malpha = .9;
  										Len = (malpha)*Len + (1-malpha)*LenL;
  										if(Len>=Longest){
                         Longest=Len;
                         chosen_s=s;
                         chosen_t=t;
   ...
   // if(n>=1000||ndraws>=5000){
  	...
  	//take away log to grt bwck
  if(n%int(nlines/30+1)==0){
  	//l1 = int(random(0,1.8));
  	//l2 = int(random(0,1.8));
