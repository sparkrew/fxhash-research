...
const PTN = 
{
     ...
     */
      ...
      * @param {Number} _density  Density of dots. default = 0.2
      */
     noiseGrad : function(_density = 0.2,_strokeW,_displace,_angle)
     {    
         const func = function(_w, _h, _rt = window)
         {
            ...
            const num = _w * _h * density/_strokeW;
            const ns = 0.01;
            ...
            for(let i = 0; i < num; i++){
                ...
                var y;
                if (_angle !== 90) {
                    x = _rt.abs(_rt.randomGaussian()) / 5 * _w * (1+_displace);
                    if(_angle == 45) {y = _rt.abs(_rt.randomGaussian()) / 2.5 * _h;}
                    else {y = _rt.random((_h));}
                }
                else if (_angle == 90) {
                    y = _rt.abs(_rt.randomGaussian()) / 5 * (_h) * (1+_displace);
                    x = _rt.random((_w));
                }
                const dia = _rt.noise(x * ns, y * ns) * 0.5 + 1;
                _rt.ellipse(x, y, dia*_strokeW, dia*_strokeW);
            }
         }
         return func;
     },
