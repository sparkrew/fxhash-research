...
void main(void){   
    ...
    float e = fbm(uv+fbm(uv*0.2-fbm(uv*2.,time*.2+10.),time*.2+200.),10.);
    float e2 = fbm(uv+fbm(uv*20.-fbm(uv*15.,time*.2+20.),time*.2+200.),200.);

         e*=1.-smoothstep(alt,alt+0.2,uv.y);


     if(bloodsky == 0.0){
       c1 = vec3(0.0,0.0,0.0);
       c2 = vec3(0.8,0.7,0.7);
       c2_2 = vec3(0.8,0.8,0.4);
       alpha = e*.7;
    }else{
      c1 = vec3(1.0,0.0,0.0);
      c2 = vec3(0.8,0.5,0.2);
      c2_2 = vec3(0.8,0.2,0.4);
       alpha = e*0.7;
    }
 ...
 //   vec3 c1 = vec3(1.0,1.0,1.0);
   // vec3 c2 = vec3(0.0,0.0,0.0);
    ...
    vec4 finf = vec4(fin,e*.7);
         finf = vec4(fin,smoothstep(0.1,1.0,alpha));
        // finf = vec4(fin,smoothstep(0.1,0.4,e));
