function ranmax(min, max){
  return Math.round(fxrand() * (max-min) + min);
}

function ranmaxdouble(min, max){
...
let clr1 = ranmax(0, 360);
let clr2 = ranmax(0, 360);
let coinflip = ranmax(0,100);


let colorMain, accentColor, accentSecond, colorMain2, accentForth, palette;

  colorMain = HSLToHEX(ranmax(0, 360), ranmax(50, 70), ranmax(30, 40));
    ...
    palette = [colorMain, accentColor, "#040406", colorMain2, "#ffffff", accentFifth];
    ...
    function setup() {
    	...
    	randomSeed(ranmax(0,1000))
    	addl1 =int(random(0,1.1));
    	addlm = int(random(0,1.1));
    	addflo =0;//int(random(0,1.02));
    	...
    	chSWbySC = int(random(0.1,1.8));
    	//print(chSWbySC);
    	chanSC=1;//int(random(0.8,1.8));
    	//scc = int(random(0,1));
      scc = 1;
    	var windowSizeChoice = int(windowWidth*windowHeight/50000);

    	if (chanSC==1){
     // I think it looks better when maxsizechoice is small
    		// it will be used later to cap random walk on sizechocie
    	minSizeChoice = 1;
    	maxSizeChoice = int(random(1,5));//min(3,max(windowSizeChoice,minSizeChoice));
    	// sample again
    	//maxSizeChoice = max(2,int(random(3,maxSizeChoice+0.1)));
    	//maxSizeChoice = int(random(2,maxSizeChoice+0.1));
    	//print(maxSizeChoice);
    	//sizeChoice=int(random(-8,maxSizeChoice+0.1));
    	//sizeChoice = max(2,sizeChoice);

    		// for demo, go bigger at min 7 (entertain). for long form, go smaller in min here
    	sizeChoice = int(random(5,maxSizeChoice+0.1)); // start at max, should get smaller
    	}else{
    	...
    	goSmallerSizeChoice = 0;int(random(0,1.1));
    	...
    	addxl = random(bufferx,centerx/1.4);
    	addxu = addxl;random(bufferx,centerx/1.2);
    	addyl = random(buffery,centery/1.4);
    	addyr = addyl;random(buffery,centery/1.2);
    	//print(addx);
    	//print(addy);
    	...
    	//track = int(random(0.8,1.8));
    	//background(255-c1+150,255-c2+150,255-c3+150);
    	background(250,255,250);
    //background(255,255,255);

    	chooserandomdist = int(random(0,1.5));
    	l1 = 0;
    	l2 = 0;//int(random(0,1.5));
    ...
    strokeWeight(sw);
                     ...
                     if(Len>=Longest){
                      ...
                      if(Len>=Longest){
                           ...
                           chosen_t=t;
    										...
    										Len = sqrt(sq((past_x +centerx)^blm*alm-(centerx+x+s)*blm) + sq((centery+past_y)*alm - (centery+y+t)*blm))
    										  if(Len>=Longest){
                           Longest=Len;
                           chosen_s=s;
                           chosen_t=t;
      ...
      y=y+chosen_t;
    	...
    	//}
    		...
    		sizeChoice = max(minSizeChoice,min(int(sizeChoice + sccc),maxSizeChoice));
    			...
    			if (nlines%100==0){
    			c1 = 255-c1;//+random([-(cc),0,cc]);
       	c2 = 255-c2;//+random([-(cc),0,cc]);
       	c3 = 255-c3;//+random([-(cc),0,cc]);
    	}
      ...
      // 	c3 = 255-c3;
        ...
        y = int(random(buffery-centery,windowHeight-centery-buffery));
      		...
      		y = y+int(random(-(sizeChoice+scb),(sizeChoice+scb)));
    			if (useaddx==1){
    				x = int(random(bufferx-centerx+addxl,windowWidth-centerx-bufferx-addxu));
        y = int(random(buffery-centery+addyl,windowHeight-centery-buffery-addyr));
    			}
     ...
     // if(n>=1000||ndraws>=5000){
    	...
    	lm=0;
    	  ...
    	  if(addl1==1){
    	l1 = int(random(0,1.001));
    	}
    	if (addlm==1){
    	lm = int(random(0,1.001));
    	}
