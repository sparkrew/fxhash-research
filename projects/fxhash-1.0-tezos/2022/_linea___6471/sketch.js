...
function setup() {
  ...
  incr = randomRange(.002,.05); 
  if (fxrand()>.8) incr = randomRange(.1,.25);
...
function initValues() {
  ...
  maxFC = randomRange(100,400);
  strokeAlpha = map(maxFC,100,400,.001,.00025);
  rot = floor(randomRange(0,4))*radians(90);
  if (incr<.01 || incr>.1) {
...
class Sheep {
  ...
  move() {
    ...
    this.vel.y = constrain(this.vel.y, -.05,.05);
