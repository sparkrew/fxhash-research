/* global preloadImagesTmr Line fxhash fxrand page PAPER Blob */
...
let paperLoaded = false // A terrible way to do this
let redLines = []
let blackLines = []
...
const drawCanvas = () => {
  ...
  ctx.fillStyle = features.paperPattern
...
const autoDownloadCanvas = async (showHash = false) => {
  ...
  element.setAttribute('download', `a-strange-and-desolate-place_${fxhash}`)
...
const downloadSVG = async (s) => {
  ...
  scaledBlackLines = page.translate(scaledBlackLines, 0.5, 0.5)
  await wrapSVG(scaledBlackLines, size, `a-strange-and-desolate-place_${fxhash}_black-lines`)
  ...
  if (redLines.length) {
    ...
    scaledRedLines = page.translate(scaledRedLines, 0.5, 0.5)
    await wrapSVG(scaledRedLines, size, `a-strange-and-desolate-place_${fxhash}_red-lines`)
  }
}
