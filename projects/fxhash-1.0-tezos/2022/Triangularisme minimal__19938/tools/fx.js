...
function rdmGradient() {
    push()
    if (grd.n < 0.5) {
        radialGradientFill(width / 2, height / 2, height * 2, width / 2, height / 2, 0, palette.bgClr, palette.clr)
        rect(width/2,height/2, width, height)
    }
    else {
        linearGradientFill(width / 2, 0, width / 2, height, palette.bgClr, palette.clr)
        rect(width/2,height/2, width, height)
    }
    pop()
}
