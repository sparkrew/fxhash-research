...
function handleUrlParams() {
  ...
  const addaptiveRatioParam = urlParams.get('adaptive')
  if (addaptiveRatioParam == null || addaptiveRatioParam == 0) {
    // print("no geratorMode specified, setting scale mode to Discover");
    switchAdaptiveRatio = -1;
  } else if (addaptiveRatioParam == 1) {
    switchAdaptiveRatio = 0;
  } else if (addaptiveRatioParam == 2) {
    switchAdaptiveRatio = 1;
  }

  const resolutionParam = urlParams.get('resolution')
  if (resolutionParam == null || resolutionParam == 0) {
    // for some reason, my trick to double the resolution to ensure sharpness on all displays
    // was very flakey on multiple iPhones I tested.  I struggled with figuring this out for several days.
    // I decided, to use 1x resolution in iphones by default and 2x everywhere else.
    if(/iPhone/i.test(navigator.userAgent)){
      resolutionMultiplier = 1
    }else{
      resolutionMultiplier = 2
    }
  } else if (resolutionParam >= 1 && resolutionParam <= 8) {
    resolutionMultiplier = parseInt(resolutionParam)
  }

  const exportParam = urlParams.get('export')
  if (exportParam == null || exportParam == 0) {
    // print("no geratorMode specified, setting scale mode to Discover");
    exportFlag = 0;
