...
function setup() {
  setupCanvas();
  /* let rdsat = int(random(0))
  let rdsat1 
  if(rdsat == 0){rdsat = 120}
  if(rdsat == 1){rdsat = 320} */
  colorMode(HSB, 350, 120, 7)
  imageMode(CENTER);
...
function draw() {
  ...
  if(rdcomp == 0){
    ...
    translate(-width/1.5, -width/1.4, -width/2.9)
     ...
     stroke(50, 200, 1)
      ...
      pop()
        ...
        for(var i = 0; i<2; i++){
          ...
          if (rdbgr == 0){fill(bgC, 100, 80)} else {fill(bgC, 100, 6)}
          noStroke()
      ...
      push()
      rotate(1.58)
      translate(-width/random(4, 5), -height/0.8, 0)
      ...
      pop()
        ...
        for(var x = 0; x< 3; x++){
          push() // arvore teste
          translate(random(-width/2, width/4), -width/random(2, 10), -width/10)
         // arvore(int(random(2)), 0.5)
          ...
          for(var x = 0; x< 7; x++){
            ...
            for(var x = 0; x< 7; x++){
              ...
              for(var x = 0; x< 7; x++){
                push()
                rotate(random(0.7, -1.6))
                translate(random(width/3, width/5.5), width/random(13, 7), width/10)
              //  arvore2(int(random(2)), random(1, 2))
                pop()
                }
     ...
     /*  push()
      //ARVORE
      translate(random(-width/3.5, -width/2), -width/2.5, -width/10)
      for(var i = 0; i< 15; i++){
        cb = 10
        cb2 = 1
        var xoff2 = 0
