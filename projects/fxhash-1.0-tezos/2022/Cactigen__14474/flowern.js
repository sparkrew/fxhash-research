function dirFl(ref, r, flc, dir, lb = 3) {
  let nogh = circFlo(ref.x, ref.y, r, flr(3, 5));
  let bgfc = flbColor[cfb];

  for (let k = 0; k < nogh.length; k++) {
    let [x, y, r] = nogh[k];
    let mlg = random(["l", "r", "u", "n"]);
    let gmn = flp(hc(x, y), r, mlg, lb);
    let mkh = flp(hc(x, y), r / 2, mlg, lb);
    fill(bgfc[0]);
    strokeWeight(1);
    for (let i = 0; i < gmn.length; i++) {
      let e = gmn[i];
      beginShape();
      for (let p of e) {
        vertex(p.x, p.y);
      }
      endShape();
    }
    fill(bgfc[1]);
    noStroke();
    for (let i = 0; i < mkh.length; i++) {
      let e = mkh[i];
      beginShape();
      for (let p of e) {
        vertex(p.x, p.y);
      }
      endShape();
    }
  }
}
function flp(ref, r, dir, lb) {
  let { center, mw, mh, bw, bh } = dirBase(ref, r, dir, lb);
  let refC = hc(center.x, center.y);
  let pl = [];
  let th = [];
  for (let i = 0; i < TAU; i += 0.1) {
    let [x, y] = mcr(ref.x, ref.y, mw, mh, i);
    pl.push([x, y]);
  }
  for (let i = 0; i < TAU; i += 0.2) {
    let [x, y] = mcr(ref.x, ref.y, bw, bh, i);
    th.push([x, y]);
  }
  pl = pl.splice(0, 20);
  th = th.splice(0, 10);
  for (let i = 0; i < pl.length; i++) {
    pl.splice(i, 1);
  }
  let gmn = [];
  for (let i = pl.length - 1; i >= 0; i--) {
    let [x, y] = pl[i];
    let [x2, y2] = i == 0 ? pl.at(-1) : pl[i - 1];
    let a = hc(x, y);
    let a2 = hc(x2, y2);
    let b = hc(th[i][0], th[i][1]);
    gmn.push(nmb(refC, a, b).reverse().concat(nmb(refC, a2, b)));
  }
  return gmn;
}

function dirBase(ref, r, dir, lb) {
  let center, mw, mh, bw, bh;
  switch (dir) {
    case "l":
      center = hc(ref.x + lb, ref.y);
      mw = r * 1.5;
      mh = r + r * 0.9;
      bw = r * 1.1;
      bh = r * 1.5;
      break;
    case "r":
      center = hc(ref.x - lb, ref.y);
      mw = r * 1.5;
      mh = r + r * 0.9;
      bw = r * 1.1;
      bh = r * 1.5;
      break;

    case "u":
      center = hc(ref.x, ref.y + lb);
      mw = r * 1.5;
      mh = r * 1.2;
      bw = r * 1.2;
      bh = r * 0.7;
      break;
    case "n":
      center = hc(ref.x, ref.y);
      mw = r * 1.5;
      mh = r * 1.2;
      bw = r * 1.2;
      bh = r;
      break;
    default:
      break;
  }
  return { center, mw, mh, bw, bh };
}
function mcr(a, b, r, tn, ang) {
  var t = tan(ang * PI);
  var px = a + (r * (1 - t ** 2)) / (1 + t ** 2),
    py = b + (tn * 2 * t) / (1 + t ** 2);
  return [px, py];
}

function nmb(a, b, c) {
  let ret = [];
  for (let t = 0; t < 1.00001; t += 0.05) {
    let x1 = lerp(a.x, b.x, t);
    let y1 = lerp(a.y, b.y, t);
    let x2 = lerp(b.x, c.x, t);
    let y2 = lerp(b.y, c.y, t);
    let x = lerp(x1, x2, t);
    let y = lerp(y1, y2, t);
    ret.push(hc(x, y));
  }
  return ret;
}

function hc(a, b) {
  return createVector(a, b);
}
