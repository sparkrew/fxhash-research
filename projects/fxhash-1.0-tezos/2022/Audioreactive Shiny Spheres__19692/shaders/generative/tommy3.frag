...
float rsphere(vec3 p,float r,float sonido){
	...
	float def = length(sin(p*2.)*msound)*.012;
		  ...
		  def = abs(length(sin(p*5.+time+sin(p*100.-time)*.5+.5)*2.-1.)*msound)*.0042;
...
float shadow(vec3 p) {
  ...
  float totdist=rdm(p.x+p.y)*3., sh=1.;
...
vec3 march(vec3 from, vec3 dir) {
  ...
  float totdist=rdm(uv.x+uv.y)*4.,d;
  for (int i=0; i<80; i++) {
    p=from+dir*totdist;
    d=de(p);
    totdist+=d;
...
void main()
{
  ...
  vec3 from = vec3(0.,mapr(rdm(sr3*10.+212.),-5.0,-7.0),-7.);
