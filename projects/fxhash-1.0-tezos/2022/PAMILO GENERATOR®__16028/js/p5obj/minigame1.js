...
//		console.log(_sp);
		...
		this.ite = _ite;
		this.rpos = [];
		let rdm2 = uniforms_fxhash.meteormaxsize;
			rdm2 = 0;
		for(let i = 0; i<15; i++){
	...
	display(_ps){
		...
		for(let i=0; i<this.rpos.length; i++){
			this.c.setAlpha(150);
			let ani = 0;
			let a = map(i, 0, this.rpos.length, 0, TWO_PI);
			let a2 = map(i, 0, this.rpos.length, 0, TWO_PI*4);
			let xx = this.rpos[i].x + sin(a + millis() * 0.0001) * uniforms_fxhash.meteormaxsize;
			let yy = this.rpos[i].y + cos(a * 4. + millis() * 0.0001) * uniforms_fxhash.meteormaxsize;


			_ps.push();
			_ps.translate(xx, yy);
			_ps.rotate(a + millis() * 0.001 + a2);
			_ps.fill(this.c);
			_ps.rect(0,0,this.s,this.s);
			_ps.fill(0,150);
			_ps.rect(0, 0, this.s * 0.75, this.s * 0.75);
			_ps.pop();
