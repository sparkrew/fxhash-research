...
void main(){
    ...
    float mprom = mapr(dprom,0.0,.05);
    float mfreq = mapr(freq,0.0,10.);

    uv1+=vec2(0.5);
    uv1*=scale(vec2(.9+sin(prom2*10.+time+prom1*5.)*mprom+mprom));
    uv1-=vec2(0.5);

    
    uv2+=vec2(0.5);
    uv2*=scale(vec2(.9+sin(prom1*10.+time+prom2*5.)*mprom+mprom));
    uv2-=vec2(0.5);
    vec4 t3 =  texture2D(tx, uv1);
	vec4 t4 =  texture2D(tx2, uv2);
	...
	vec3 fin = mix(t3.rgb,t4.rgb,sin(t3.rgb*100.)*.5+.5);
