...
// 'Blur of Alternation - Chromatic Distribution'

// image parameters
let cw = ch = 512;
let mo = Math.floor(fxrand() * 6) + 2 // alternator module: [2, 3, 4, 5, 6, 7]
let cra = 255 - 256%mo - 1; // color range
let cr = Math.floor(fxrand() * cra); // red value
let cg = Math.floor(fxrand() * cra); // green value
...
let c = Math.floor(fxrand() * 3) - 1; // angle: 45: -1, 1; 180: 0;
let d = Math.floor(fxrand() * 2) * 2 - 1; // polydiagonality side: left: -1; right: 1;
let e = Math.floor(fxrand() * 2) * 2 - 1; // direction (speed): left: -1; right: 1;
...
window.$fxhashFeatures = {
  ...
  "Input saturation": getFeatureString9(gd)
}