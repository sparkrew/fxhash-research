		...
		if (RM.objts.length > 0) {
			if (RM.objts[RM.activeRender].localUniformsNames.length > 0) {
				for (let i = 0; i < RM.objts[RM.activeRender].localUniformsNames.length; i++) {
					let sl = new Slider(60, yy, RM.objts[RM.activeRender].localUniformsNames[i], RM.objts[RM.activeRender].localUniformsValues[i]);
					yy += 60;
					this.sliders.push(sl);
				}
			}
			for (let i = 0; i < RM.objts.length; i++) {
				let x = windowWidth * .25;
				let y =  i * 50 + 30;
				//var cajita = ;
...
class Slider {
	...
	draw() {
		...
		if (this.isFxHashControlled) {
			if (this.overRect(mx, my, xx, yy, this.w, this.h)) {
				fill(255,0,0);
				if (mouseIsPressed) {
