...
function keyPressed() {
    if (key === 's' || key === 'S') {
      saveCanvas(title, 'png'); // guarda el canvas como un archivo png
    }
    
    key === '1' ? filter(GRAY)         :null;
    key === '2' ? filter(POSTERIZE, 2) :null;
    key === '3' ? filter(THRESHOLD)    :null;

    key === 'r' || key === 'R' ? restart():null;
}
